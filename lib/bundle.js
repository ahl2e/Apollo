...
class Game{
  ...
  preview(ctx){
    ...
    ctx.fillStyle = "white";
    ctx.font = "25px Andale Mono";
    ctx.fillText('Directions', 200,200);
    ctx.font = "17px Andale Mono";
    ctx.fillText('1) Find a suitable flat landing zone.', 200, 230);
    ctx.fillText("2) If you pass you landing zone, don't worry.  The moon is a circle.  You'll come back around."  , 200, 260);
    ctx.fillText('3) Slow down to fall out of orbit.', 200, 290);
    ctx.fillText('4) Come down softly and land with less than 10 m/s of vertical velocity.', 200, 320);
    ctx.fillText('5) Land three times without crashing to qualify for launch.', 200, 350);
...
class GameView{
  ...
  showLanding(){
    ...
    this.game.surfaceDraw(this.ctx);
    if (this.lander.landings == 3){
      this.ctx.fillStyle = "green";
      this.ctx.font = "50px Andale Mono";
      this.ctx.fillText(`You're Ready to Launch, Commander`,200,300);
    }else{
      setTimeout(this.resetLander.bind(this),3000);
    }
  }
  ...
  showCrash(){
    ...
    this.ctx.font = "50px Andale Mono";
    this.ctx.fillText(`Mission Failure`,300,300);
    this.ctx.fillStyle = "white";
    this.ctx.font = "30px Andale Mono";
    this.ctx.fillText(`Press Space to Try Again`,300,400);
    this.ctx.closePath();
    this.game.surfaceDraw(this.ctx);
